...
def purchase_ticket(event):
    try:
        ...
        subprocess.Popen([sys.executable, ticket_path,
                          event["title"], event["date"], event["time"], 
                          event["location"], str(event["ticket_price"]), 
                          str(event["available_tickets"])]).wait()
    except Exception as e:
        show_error(f"Error launching ticket.py: {e}")
...
def handle_menu_click(item):
    try:
        if item == "Tickets":
            ...
            subprocess.Popen([sys.executable, main_path]).wait()
        else:
